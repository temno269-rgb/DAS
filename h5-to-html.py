...
def build_interactive_3d(
    ...
    # ===== МАСТЕР-ДАТАСЕТ =====
    MASTER_MAX_DIM = 600
    step_win_m = max(1, n_windows // MASTER_MAX_DIM)
    step_chan_m = max(1, n_channels // MASTER_MAX_DIM)
    ...
    for m in available_metrics_list:
        ...
        if arr.shape == (n_windows, n_channels):
            ...
            # Base64-кодирование: rows|cols|base64data — в 4× компактнее JSON
            b64 = base64.b64encode(sub.tobytes()).decode('ascii')
            all_metrics_master[m] = f"{sub.shape[0]}|{sub.shape[1]}|{b64}"
    ...
    def _decode_b64_f32(encoded: str) -> np.ndarray:
        """Декодирует Base64 float32 строку → 2D numpy массив."""
        parts = encoded.split('|')
    ...
    Z_init_full = _decode_b64_f32(all_metrics_master[metric_key])
    ...
    SCRIPT_VERSION = "v4.4-dark-walls"
  ...
  window.masterMaxDim = {max(master_n_win, master_n_chan)};
  ...
  window.defaultRes2D = {max(master_n_win, master_n_chan)};
...
(function(){
  ...
  // === Декодирование Base64 float32 данных ===
  function decodeF32Array(encoded) {
    var parts = encoded.split('|');
    var rows = +parts[0], cols = +parts[1];
    ...
    for (var r = 0; r < rows; r++) {
      result.push(Array.from(f32.subarray(r * cols, (r + 1) * cols)));
    }
    return result;
  }
  ...
  function decodeMasterData() {
    ...
    for (var ki = 0; ki < keys.length; ki++) {
      ...
      if (typeof val === 'string' && val.indexOf('|') > 0) {
        window.masterData[key] = decodeF32Array(val);
      }
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Интерактивный 3D/2D график из .h5 артефактов с динамическим прореживанием."
    )
    parser.add_argument(
        "h5_file",
        nargs="?",
        default=None,
        help="Путь к .h5-файлу. Если не указан, автоматически ищется последний изменённый .h5 в текущей директории.",
    )
    parser.add_argument(
        "-o", "--output",
        default="activity_3d.html",
        help="Имя выходного HTML-файла (по умолчанию: activity_3d.html).",
    )
    parser.add_argument(
        "--metric",
        default="mean",
        help="Матрица из .h5 для оси Z (по умолчанию: mean). Автоматически выбирает из доступных в файле.",
    )
    parser.add_argument(
        "--demo",
        action="store_true",
        help="Создать синтетический файл demo_activity.h5 и запустить демонстрацию.",
    )
    ...
    if args.demo:
        ...
        if h5_file_path is None:
            cwd_h5s = list(Path(".").glob("*.h5"))
            if cwd_h5s:
                cwd_h5s.sort(key=lambda p: p.stat().st_mtime, reverse=True)
                h5_file_path = cwd_h5s[0]
                print(f"[INFO] Автоматически выбран: {h5_file_path.name}")
            else:
                script_parent_h5 = Path(__file__).resolve().parent / "activity.h5"
                if script_parent_h5.exists():
                    h5_file_path = script_parent_h5
                else:
                    print("[ERROR] Не указан путь к файлу .h5, и в текущей директории отсутствуют .h5 файлы.")
                    print("        Использование: python3 h5-to-html.py <путь_к_файлу>.h5")
                    print("        Или запустите демо: python3 h5-to-html.py --demo")
                    sys.exit(1)

        h5_path = Path(h5_file_path)
        if not h5_path.exists():
            print(f"[ERROR] Файл не найден: {h5_path}")
            sys.exit(1)

        build_interactive_3d(h5_path, output_html=args.output, metric_key=args.metric)
